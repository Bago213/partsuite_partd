...
// newAddressPubKey returns a new partutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) partutil.Address {
	addr, err := partutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new partutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) partutil.Address {
	addr, err := partutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new partutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) partutil.Address {
	addr, err := partutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the partutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the partutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := partutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := partutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := partutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := partutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := partutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*partutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*partutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*partutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*partutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*partutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
