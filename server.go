...
import (
	...
	"github.com/particl/partsuite_partd/addrmgr"
	...
	"github.com/particl/partsuite_partd/connmgr"
	...
	"github.com/particl/partsuite_partd/mining/cpuminer"
	"github.com/particl/partsuite_partd/netsync"
	...
	"github.com/particl/partsuite_partutil/bloom"
)
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a partutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := partutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a partutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := partutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > partutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, partutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *partutil.Tx) {
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *partutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
