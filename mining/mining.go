...
const (
	...
	MinHighPriority = partutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr partutil.Address) (*partutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *partutil.Tx, height int32) error {
...
func logSkippedDeps(tx *partutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress partutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*partutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := partutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := partutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A partutil.Block should use saved in the state to avoid
