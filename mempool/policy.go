...
const (
	...
	DefaultMinRelayTxFee = partutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee partutil.Amount) int64 {
	...
	if minFee < 0 || minFee > partutil.MaxSatoshi {
		minFee = partutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *partutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee partutil.Amount) bool {
...
func checkTransactionStandard(tx *partutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee partutil.Amount,
...
func GetTxVirtualSize(tx *partutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
