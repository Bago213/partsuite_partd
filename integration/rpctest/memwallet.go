...
import (
	...
	"github.com/particl/partsuite_partd/rpcclient"
	...
	"github.com/particl/partsuite_partutil/hdkeychain"
)
...
type memWallet struct {
	...
	addrs map[uint32]partutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]partutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*partutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          partutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (partutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []partutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt partutil.Amount, feeRate partutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := partutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate partutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate partutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt partutil.Amount
	for _, output := range outputs {
		outputAmt += partutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() partutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance partutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (partutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := partutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
