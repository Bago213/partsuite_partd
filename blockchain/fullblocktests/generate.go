...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: partutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := partutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*partutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, partutil.NewTx(tx))
	}
...
func additionalCoinbase(amount partutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee partutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee partutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee partutil.Amount) *wire.MsgTx {
